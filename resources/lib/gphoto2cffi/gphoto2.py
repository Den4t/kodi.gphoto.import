...
class Directory(object):
    ...
    #def __del__(self):
    #    print('>>>> Directory Destructor for %s'%self.__class__.__name__)
...
class File(object):
    ...
    def iter_data(self, chunk_size=2**16, ftype='normal'):
        ...
        offset_p[0]=0
        while offset_p[0] < self.size:
            if offset_p[0] + chunk_size < self.size:
                sz=chunk_size
            else:
                sz=self.size-offset_p[0]
            size_p[0] = sz
...
#            yield ffi.buffer(buf_p, size_p[0])[:]
    ...
    def _info(self):
        if self.__info is None:
            ...
            try:
                ...
                #!!!this is addiing a lot of thtottiling
    ...
    def __repr__(self):
        return "File(\"{0}/{1}\")".format(self.directory.path.rstrip("/"),
                                          self.name)
    #def __del__(self):
    #    print('>>>> File Destructor for %s'%self.__class__.__name__)
